...
def test_one_epoch(model, criterion, data_loader, device, logger, args, epoch, nprocs=4):
    ...
    for camera_inputs_val, motion_inputs_val, enc_target_val, distance_target_val, class_h_target_val, dec_target in metric_logger.log_every(data_loader, 500, header):
        ...
        if args.distributed:
            ...
            # prob_val = enc_score_p0.detach().cpu().numpy()  # enc_score_p0[:, :7].cpu().numpy()
            all_probs.extend(enc_score_p0[:, :].detach().cpu().numpy())  # (89, 7)  # all_probs += list(prob_val)
            # t0_class_batch = class_h_target.detach().cpu().numpy()  # class_h_target[:, :7].cpu().numpy()
            all_classes.extend(class_h_target[:, :].detach().cpu().numpy())
            # set_trace()
        else:
            # prob_val = enc_score_p0[:, :7].cpu().numpy()

            prob_val = F.softmax(enc_score_p0[:, :], dim=-1).cpu().numpy()
            # prob_val = F.softmax(enc_score_p0, dim=-1)[:, :7].cpu().numpy()
            all_probs += list(prob_val)  # (89, 7)
            # dec_score_metrics = dec_scores[:, :7].cpu().numpy()
            dec_score_metrics += list(F.softmax(dec_scores.view(-1, num_class)[:, :], dim=-1).cpu().numpy())
            # dec_score_metrics += list(F.softmax(dec_scores.view(-1, num_class), dim=-1)[:, :7].cpu().numpy())
    ...
    if args.distributed:
        ...
        if utils.is_main_process():
            ...
            logger.output_print(str(all_probs.shape))  # (7, 180489)

            all_classes = np.asarray(all_classes).T
            logger.output_print(str(all_classes.shape))  # (7, 180489)
