...
def eval_experiment(exp_dir, save_results=True, save_fig=True, checkpoint='DEFAULT'):
    ...
    model_dict = generate_dict(os.path.join(exp_dir, 'log_dist.txt'))
    args = ModelConfig(**model_dict)
...
def compare_experiments(root_dir, labels = ['OverTaking', 'LaneChange', 'WrongLane', 'Cutting'], save=True):
    ...
    for root, subdirs, files in sorted(os.walk(root_dir)):
        if 'checkpoint.pth' in files:
            # print(root)
            try:
                dir_metrics = eval_experiment(root)
            except:
                continue
            # set_trace()
