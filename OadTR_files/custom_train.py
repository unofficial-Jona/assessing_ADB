...
def evaluate(model, criterion, data_loader, device, logger, args, epoch, nprocs=4):
    ...
    for camera_inputs_val, motion_inputs_val, enc_target_val, distance_target_val, class_h_target_val, dec_target in metric_logger.log_every(data_loader, 500, header):
        ...
        if args.distributed:
            ...
            all_probs.extend(enc_score_p0[:, :8].detach().cpu().numpy())  # (89, 8)  # all_probs += list(prob_val)
            # t0_class_batch = class_h_target.detach().cpu().numpy()  # class_h_target[:, :8].cpu().numpy()
            all_classes.extend(class_h_target[:, :8].detach().cpu().numpy())
            # set_trace()
        else:
            # prob_val = enc_score_p0[:, :8].cpu().numpy()
            prob_val = F.softmax(enc_score_p0[:, :8], dim=-1).cpu().numpy()
            # prob_val = F.softmax(enc_score_p0, dim=-1)[:, :8].cpu().numpy()  
            all_probs += list(prob_val)  # (89, 8)
            # dec_score_metrics = dec_scores[:, :8].cpu().numpy()
            dec_score_metrics += list(F.softmax(dec_scores.view(-1, num_class)[:, :8], dim=-1).cpu().numpy())
            ...
            dec_target_metrics += list(dec_target.view(-1, num_class)[:, :8].cpu().numpy())
            for iii in range(num_query):
                dec_score_metrics_every[str(iii)] += list(F.softmax(dec_scores[:,iii,:].view(-1, num_class)[:, :8], dim=-1).cpu().numpy())
                dec_target_metrics_every[str(iii)] += list(dec_target[:,iii,:].view(-1, num_class)[:, :8].cpu().numpy())
        # metric_logger.update(class_error=loss_dict_reduced['class_error'])
